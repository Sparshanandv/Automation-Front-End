...
import { Feature, FeatureStatus, StatusHistoryEntry, TestCase, PullRequest } from '../../types'
import { featureService } from '../../services/feature.service'
import { qaService } from '../../services/qa.service'
import { aiService } from '../../services/ai.service'
import { prService } from '../../services/pr.service'
import { STATUS_ORDER, STATUS_LABELS, getNextStatus, validateFeatureMove } from '../../utils/statusUtils'
...
export default function FeatureDetailPage() {
  ...
  useEffect(() => {
    ...
    if (PLAN_STATUSES.includes(feature.status)) {
      ...
      if (['PLAN_APPROVED', 'CODE_GEN', 'PR_CREATED', 'DONE'].includes(feature.status)) {
        aiService.getCodeGeneration(id)
              ...
              <div className="grid gap-6 xl:grid-cols-[1.45fr_1fr]">
                ...
                <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-1 min-w-0 break-words">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                            feature.type === 'bug' ? 'bg-red-100 text-red-700' :
                  ...
                  {pullRequest && (
                    <div className="bg-white rounded-2xl border border-gray-200 p-6">
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Pull Request</p>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${getPRStatusColor(pullRequest.status)}`}>
                          {pullRequest.status}
                        </span>
                      </div>
                      <div className="space-y-2">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <h3 className="font-semibold text-gray-900 mb-2">{pullRequest.title}</h3>
                            <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
                              <span className="font-mono">#{pullRequest.pr_number}</span>
                              <span>{pullRequest.repository.owner}/{pullRequest.repository.name}</span>
                            </div>
                            <p className="text-xs text-gray-500 font-mono">{pullRequest.branch_name}</p>
                          </div>
                          <a
